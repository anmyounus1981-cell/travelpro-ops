...
import { getDashboard, getOperations } from "@/lib/data/operations";
...
<aside><div className="brand">TP<span>TravelPro Ops</span></div><nav>{navItems.map((item, i) => <a key={item.label} className={i === 0 ? "active" : ""} href={item.href}>{item.label}</a>)}</nav><p className="guardrail">Human-controlled payment & ticketing</p><form action={logout}><button type="submit">Sign out</button></form></aside>
...
<section id="new-case" className="panel form-panel"><div><p className="eyebrow">CASE INTAKE</p><h2>Create a travel case</h2><p>Record the inquiry now. Fare search, PNR creation, and ticketing remain manual.</p></div><form action={createCase} className="grid-form"><label>Corporate client<select required name="client_id"><option value="">Select client</option>{data.clients.map(c=><option key={c.id} value={c.id}>{c.company_name}</option>)}</select></label><label>Origin<input name="origin" required maxLength={3} placeholder="DAC"/></label><label>Destination<input name="destination" required maxLength={3} placeholder="DXB"/></label><label>Departure<input name="departure_date" type="date" required/></label><label>Return<input name="return_date" type="date"/></label><label>Trip type<select name="trip_type"><option value="roundtrip">Round trip</option><option value="oneway">One way</option><option value="multicity">Multi-city</option></select></label><label>Passengers<input name="passenger_count" type="number" min="1" defaultValue="1" required/></label><label>Cabin<select name="cabin_class"><option>Economy</option><option>Premium Economy</option><option>Business</option><option>First</option></select></label><label>Source<select name="intake_source"><option value="manual">Manual</option><option value="whatsapp">WhatsApp</option><option value="email">Email</option><option value="web">Web</option></select></label><label className="wide">Notes<textarea name="notes" rows={3}/></label><button className="primary wide">Create case</button></form></section>
<section id="workflow" className="workflow"><p className="eyebrow">CONTROLLED WORKFLOW</p><h2>Case → Quote → Booking → Payment → Ticket</h2><p>AI may extract and draft. Only an authorized human verifies funds or records a completed GDS transaction.</p></section><Workflow data={data} ops={ops}/></main></div>}
