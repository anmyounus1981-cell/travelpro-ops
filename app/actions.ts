...
export async function verifyPayment(fd:FormData){const db=await createClient(),id=text(fd,"id"),status=text(fd,"status")||"verified";const{data:p}=await db.from("payments").select("evidence_file_path").eq("id",id).single();if(!p?.evidence_file_path)throw new Error("No payment evidence on file");const{error}=await db.from("payments").update({status,verified_by:"00000000-0000-0000-0000-000000000001",verification_note:text(fd,"verification_note")}).eq("id",id);if(error)throw error;await audit(`payment.${status}`,"payment",id,{note:text(fd,"verification_note")});revalidatePath("/")}
...
export async function createReminder(fd:FormData){const db=await createClient();const{data,error}=await db.from("reminders").insert({entity_type:text(fd,"entity_type"),entity_id:text(fd,"entity_id"),reminder_type:text(fd,"reminder_type"),due_at:new Date(text(fd,"due_at")).toISOString(),message:text(fd,"message")}).select("id").single();if(error)throw error;await audit("reminder.scheduled","reminder",data.id);revalidatePath("/")}
